        ...
        # try:
            if request.user.is_authenticated:
                ...
                if request.META.get("HTTP_X_REQUESTED_WITH")=="XMLHttpRequest":
                    ...
                    if UserUploadedImage.objects.filter(user=request.user):
                        image_list=[]
                        image_dict={}

                        for image_obj in UserUploadedImage.objects.filter(user=request.user):

                            image_dict["id"]=image_obj.id
                            image_dict["url"]=image_obj.image.url
                            image_dict["text"]=image_obj.text
                ...
                context = {'segment': 'images'}

                if UserRestInfo.objects.filter(id=request.user.id).exists():

                    context['user_rest_info'] = UserRestInfo.objects.get(id=request.user.id)

                    html_template = loader.get_template('home/images.html')
